...
// gcc parallel_min_max.c find_min_max.c utils.c -o parallel_min_max.out && ./parallel_min_max.out --seed 1 --array_size 5 --pnum 1 --by_files --timeout 10
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < pnum; i++) {
    ...
    if (child_pid[i] >= 0) {
      ...
      if (child_pid[i] == 0) {
        ...
        if (with_files) {
          FILE *f;
          f=fopen("parallel_min_max.txt","w");
          char buf[33];
  ...
  for (i = 0; i < pnum; i++) {
    ...
    if (with_files) {
      FILE *f = fopen("parallel_min_max.txt","r");
      char buf[256];
      int a;
      fscanf(f,"%s",buf);
      a=atoi(buf);
